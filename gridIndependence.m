...
Ipox_exp = Material(1,1);
Ipred_exp = Material(1,2);
Epp_exp = Material(1,3);

%% governing equation paramters (fixed)
D = 1.07E-05;                  % Diffusivity (cm2/s)
Cb = 1E-05;                    % Bulk concentration (mol/cm3)
Ae = 0.088;

%% Parameter of Butler-Volmer equation (fixed)
k0 = 4.67E-3;                  % Rate constant (cm/s)
alfaA = 0.433;                % Cathodic charge transfer coefficient
alfaC = 1-alfaA;             % Anodic charge transfer coefficient
Ef0 = Material(1,4);         % reference electrode potential (volt)
...
L = 6*sqrt(D*tmax);         % Span of simulation d,omain (cm)
